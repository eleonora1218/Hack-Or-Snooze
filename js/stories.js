...
function putUserStoriesOnPage() {
  ...
  if (currentUser.ownStories.length === 0) {
    $ownStories.append("<h5>You have not submitted any stories</h5>");
  } else {
...
function putFavoritesListOnPage() {
  ...
  if (currentUser.favorites.length === 0) {
    $favoritedStories.append("<h5>You have no favorite stories</h5>");
  } else {
...
//When "hack or snooze" in nav links is clicked, removes fave stories and own stories if open
function clear() {
  $ownStories.hide();
  $favoritedStories.hide()
}
$hackOrSnoozeLink.on('click', clear);

//when own stories is selected, removes my faves if open
function hideFaves() {
  $favoritedStories.hide();
}
$myStoriesLink.on('click', hideFaves);

//when my faves is selected, removes own stories if open
function hideMyStories() {
  $ownStories.hide();
}
$myFaveLinks.on('click', hideMyStories);
